...
class Configs: 
    ...
    def __init__(
        ...
        self.NetWork1.load_state_dict(torch.load('star.pth'))
        self.NetWork2.load_state_dict(torch.load('roma.pth'))
    ...
    def show_generate(self, title="Gen"): 
        ...
        with torch.no_grad(): 
            ...
            for t_ in range(self.n_steps):
                t = self.n_steps - t_ - 1 
                if t_ < (self.n_steps // 2):
                    x = self.ddpm1.p_sample(x, x.new_full((self.n_samples,), t, dtype=torch.long))   
                else: 
                    x = self.ddpm2.p_sample(x, x.new_full((self.n_samples,), t, dtype=torch.long))   
